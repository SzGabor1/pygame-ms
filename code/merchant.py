...
class Merchant(NPC):
    def __init__(self, npc_name, pos, groups, obstacle_sprites, id, item_list):
        ...
        self.shop = Shop(self.itemlist, self.toggle_shop, self.name)

    def show_shop_button(self):
        if self.range_of_player and not self.is_shop_open:
            self.shop.display_shop_button()

    def show_shop(self, player):
        if self.range_of_player and self.is_shop_open:
